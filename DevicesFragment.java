package com.itoneclick.buypassnow.society.bluetooth_le;
...
import com.itoneclick.buypassnow.BPNApplication;
...
public class DevicesFragment extends ListFragment {
    ...
    private static final long LESCAN_PERIOD = 5000; // similar to bluetoothAdapter.startDiscovery
    ...
    public DevicesFragment() {
        leScanCallback = (device, rssi, scanRecord) -> {
            if (device != null && DevicesFragment.this.getActivity() != null) {
                DevicesFragment.this.getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        DevicesFragment.this.updateScan(device);
                    }
                });
            }
        };
    ...
    public void onResume() {
        ...
        if (bluetoothAdapter == null) {
            ...
            setEmptyText("Bluetooth is disabled, please enable it from bluetooth settings");
            if (menu != null) {
                listItems.clear();
    ...
    private void updateScan(BluetoothDevice device) {
        ...
        if (listItems.indexOf(device) < 0) {
            if (Constant.BLUETOOTH_LE_DEVICE_NAME.equals(device.getName()) || Constant.BLUETOOTH_LE_DEVICE_ADDRESS.equals(device.getAddress())) {
        ...
        /*if (Constant.BLUETOOTH_LE_DEVICE_NAME.equals(device.getName()) || Constant.BLUETOOTH_LE_DEVICE_ADDRESS.equals(device.getAddress())) {
            ...
            sendResultToActivity(device.getAddress());
        } else
            BPNApplication.bluetoothDeviceNotFound = true;*/
    ...
    private void sendResultToActivity(String deviceAddress) {
        Intent intent = new Intent();
        intent.putExtra("bAddress", deviceAddress);
